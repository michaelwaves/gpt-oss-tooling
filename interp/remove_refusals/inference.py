...
def chat_with_model():
    ...
    while True:
        try:
            prompt = input("User: ")
            if prompt.startswith(":vector"):
                try:
                    active_vector_idx = int(prompt.split()[1])
                    print(f"Now using refusal_vectors[{active_vector_idx}]")
                except:
                    print("Usage: :vector <idx>")
                continue
                ...
                for layer_idx, layer in enumerate(layers):
                    if layer_idx < len(refusal_vectors):
                        ...
                        ablated_output = direction_ablation_hook(
                            layer.output,
                            refusal_vectors[active_vector_idx].to(
                                layer.output.device)
                        )
                        if cfg.debug:
                            print(
